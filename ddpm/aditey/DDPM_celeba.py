...
class SinusoidalEmbeddings(torch.nn.Module):
    ...
    def __init__(self, time_steps:int = 1500, embedding_dimension: int = 512):
        super().__init__()
...
class UNET(nn.Module):
    ...
    def __init__(self, Channels: list = [64, 128, 256, 512, 512, 256, 128, 64], Attentions: list = [False, False, True, False, False, False, True, False],
            Upscales: list = [False, False, False, False, True, True, True, True], num_groups: int = 32, dropout_prob: float = 0.1, num_heads: int = 8, 
            input_channels: int = 3, output_channels: int = 3, time_steps: int = 1500, kernel_size: int = 3, padding: int = 1):
...
class DDPM_Scheduler(nn.Module):
    ...
    def __init__(self, num_time_steps: int = 1500, s: float = 0.008):
        super().__init__()
...
def sample_images(model, scheduler, device, num_time_steps, epoch, save_dir="celeba-images"):
...
def inference_mod(ema_model, scheduler, epoch: int, num_time_steps: int = 1500):
    ...
    with torch.no_grad():
        ...
        for index in range(10):
            ...
            for timestep in reversed(range(1, num_time_steps)):
                ...
                epsilon_noise = torch.randn(1, 3, 128, 128).to(device)
            ...
            os.makedirs("intermediate-images", exist_ok = True)
            
            plt.imsave(f"intermediate-images/epoch_{epoch}_sample_{index}.png", new_image_save)



def train(batch_size: int = 128, num_time_steps: int = 1500, num_epochs: int = 200, seed: int = -1, ema_decay: float = 0.9999, lr: float = 2e-4, checkpoint_path: str = "", csv_log_path: str = "loss_log.csv"):
    set_seed(random.randint(0, 2**32-1)) if seed == -1 else set_seed(seed)

    transform = transforms.Compose([ transforms.CenterCrop(148), transforms.Resize((128, 128)), transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))])
    
    train_dataset = datasets.ImageFolder(root='./data/celeba', transform=transform)
    ...
    os.makedirs('celeba-checkpoints', exist_ok=True)
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        checkpoint = {'epoch': epoch + 1, 'weights': model.state_dict(), 'optimizer': optimizer.state_dict(), 'ema': ema.state_dict()}

        torch.save(checkpoint, 'celeba-checkpoints/ddpm_checkpoint.pth')
        torch.save(checkpoint, f'celeba-checkpoints/ddpm_checkpoint_epoch_{epoch+1}_t_1500.pth')
        



def inference(checkpoint_path: str = "", num_time_steps: int = 1500, ema_decay: float = 0.9999):
    checkpoint = torch.load(checkpoint_path, map_location=device)
    ...
    with torch.no_grad():
        ...
        for index in range(30):
            ...
            os.makedirs("celeba-images", exist_ok=True)
            plt.imsave(f"celeba-images/sample_{index}.png", new_image_save)
            
def main():
    train(checkpoint_path = 'celeba-checkpoints/ddpm_checkpoint.pth', lr = 1e-4, num_epochs = 2000, batch_size = 16)
    inference('celeba-checkpoints/ddpm_checkpoint.pth')
