...
def histogram(sample_points, num_time_steps):
    ...
    plt.savefig(f"graphs/hist_T_{num_time_steps}.png", dpi = 300, bbox_inches = 'tight')
    plt.close()
...
def plot_metrics(compiled_list):
    ...
    plt.ylabel("Mean")
    plt.title("Mean vs T")
    plt.savefig("graphs/mean_vs_T.png", dpi = 300, bbox_inches = 'tight')
    plt.close()
    ...
    plt.savefig("graphs/variance_vs_T.png", dpi = 300, bbox_inches = 'tight')
    plt.close()
    ...
    plt.savefig("graphs/kl_vs_T.png", dpi = 300, bbox_inches = 'tight')
    plt.close()
    ...
    plt.savefig("graphs/wasserstein_vs_T.png", dpi = 300, bbox_inches = 'tight')
    plt.close()
    ...
    plt.savefig("graphs/ks_vs_T.png", dpi = 300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig("graphs/tv_vs_T.png", dpi = 300, bbox_inches='tight')
    plt.close()
    ...
    plt.savefig("graphs/skewness_vs_T.png", dpi = 300, bbox_inches='tight')
    plt.close()
...
def sample(model, scheduler, num_steps, n_samples):
    ...
    with torch.no_grad():
        ...
        for i in range(len(indices)):
            t_idx = indices[i].item()
            t_tensor = torch.full((n_samples,), t_idx, device=device, dtype=torch.long)

            predicted_noise = model(x, t_tensor)

            alpha_bar_t = scheduler.alpha[t_idx]
            beta_t = scheduler.beta[t_idx]
            alpha_t = 1 - beta_t
...
def compute_fid(model, scheduler, dataloader, num_steps, n_samples = 5000):
    ...
    for batch_pos in range(0, n_samples, batch_size):
...
if __name__ == "__main__":
    ...
    checkpoint = torch.load("mnist-checkpoints/ddpm_checkpoint.pth", map_location=device)
    ...
    plt.savefig("graphs/fid_vs_T.png", dpi = 300, bbox_inches = 'tight')
    plt.close()
